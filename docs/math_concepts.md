...
| # | Concept | What it adds | Replaces/Enhances | Priority |
|---|---------|-------------|-------------------|----------|
| 10 | **Persistent Homology (TDA)** | Topological Data Analysis — tracks "shape" of data as it evolves. Persistence diagrams show stable vs dying topological features. Applied to financial crash prediction, cardiac arrhythmia. | New operator: topological coherence | **Done** — `src/delta72/tda.py` |
...
| 14 | **Granger Causality** | Statistical test for whether one time series helps predict another. Multi-building: does coherence loss in building A predict loss in building B? | Causal structure discovery between systems | **Done** — `src/delta72/granger.py` |
...
| 17 | **Stochastic Resonance** | Noise can amplify weak signals in nonlinear systems. Relevant because Δ shows threshold behavior — there may be an optimal noise level where detection is enhanced. | Connects to the 72 threshold concept | **Done** — `src/delta72/stochastic_resonance.py` |
